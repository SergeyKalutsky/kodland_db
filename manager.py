...
class DataBase:
    ...
    def put(self, tablename, data):
        sess.add(self.t[tablename](**data))
        sess.commit()

    def delete(self, tablename, key, val):
        sess.query(self.t[tablename]).filter(
            getattr(self.t[tablename], key) == val).delete()
        sess.commit()

    def get(self, tablename, key, val):
        return sess.query(self.t[tablename]).filter(getattr(self.t[tablename], key) == val).first()

    def get_all(self, tablename):
        return sess.query(self.t[tablename]).all()
